  ...
  export default function JournalPage() {
    ...
    try {
      ...
      const parchases =
        parchaseRes.status === "fulfilled"
          ? parchaseRes.value.data ?? []
          : [];
    ...
    // ================= COMBINED ROWS =================
  // ================= COMBINED ROWS (اصلاح شده طبق ژورنال) =================
 // ================= COMBINED ROWS (اصلاح شده با شماره فروش و نسخه) =================
const combinedRows = useMemo(() => {
  ...
  return journals.map((j) => {
    ...
    // ==== اصلاح فروش: شماره فروش در توضیحات اضافه شود ====
    if (j.ref_type === "sale") {
      description = `فروش شماره ${j.ref_id}`;
    }

    // ==== اصلاح نسخه: باقی‌مانده صفر و شماره نسخه در توضیحات ====
    ...
    return {
      id: j.id,
      date: j.journal_date
    ? new Date(j.journal_date).toLocaleDateString("fa-IR", {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
      })
