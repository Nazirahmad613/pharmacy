...
export default function ParchaseForm() {
  ...
  const handleSavePurchase = async () => {
    ...
    const payload = {
      parchase_date:
        parchaseDate || new Date().toISOString().split("T")[0],
      par_paid,
    ...
    try {
      ...
      const purchaseId =
        res?.data?.parchase_id ?? "-";
      const supplier = suppliers.find(
        s => Number(s.reg_id) === Number(selectedSupplier)
      );

      const itemsCopy = purchasedItems.map(i => ({ ...i }));
            ...
            <div className="form-grid">
              <div>
                ...
                <input
                  ...
                  value={parchaseDate}
                  onChange={e =>
                    setParchaseDate(e.target.value)
                  }
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  value={par_paid}
                  onChange={e =>
                    setParPaid(Number(e.target.value))
                  }
                />
              </div>
