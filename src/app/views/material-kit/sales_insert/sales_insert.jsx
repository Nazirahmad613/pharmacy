...
export default function SaleForm() {
  ...
  // ===== اضافه کردن دو فیلد جدید =====
  const [paidSales, setPaidSales] = useState(0);
  const [dueSales, setDueSales] = useState(0);
  ...
  useEffect(() => {
    const due = Number(netSales) - Number(paidSales || 0);
    setDueSales(due >= 0 ? due : 0);
  }, [netSales, paidSales]);
  ...
  const handleSaveSale = async () => {
    ...
    const payload = {
      ...
      discount,
      net_sales: netSales,
      paid_sales: paidSales, // اضافه شد
      due_sales: dueSales,   // اضافه شد
  ...
  return (
    <MainLayoutpur>
      ...
      <div className="form-container">
        ...
        <input type="number" value={paidSales} onChange={e => setPaidSales(Number(e.target.value))} />

        <label>مبلغ باقی‌مانده</label>
        <input type="number" value={dueSales} readOnly />
      </div>

      {/* ================= فرم آیتم‌ها ================= */}
      <form className="medication-page" style={{ display: "flex", gap: "10px", flexWrap: "wrap" }} onKeyDown={handleKeyDown}>
        <div>
          <label>کتگوری</label>
